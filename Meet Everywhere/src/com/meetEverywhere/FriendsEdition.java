    ...
    private void getContacts(){
		...
		if(dbCursor.moveToFirst()) {
			do {
				...
				fullContacts.add(new Contact(id, name, num));
        ...
        public boolean onOptionsItemSelected(MenuItem item) {
            ...
            case R.id.save:
            	...
            	for(int i = 0 ; i < fullContacts.size() ; i++){
            		...
            		if(positions.get(i)){
            		   if(c.getId() < 0)
            			   myDBAdapter.insertContact(c.getName(), c.getNumber());
            		} else if(c.getId() >= 0)
            			myDBAdapter.deleteContact(c.getId());
            	}
            ...
            case (PICK_CONTACT) :
              if (resultCode == Activity.RESULT_OK) {
                ...
                if (c.moveToFirst()) {
                  ...
                  String hasPhoneNumber = c.getString(c.getColumnIndexOrThrow(ContactsContract.Contacts.HAS_PHONE_NUMBER));
                  if (Integer.parseInt(hasPhoneNumber) > 0){
                	  ...
                	  Toast.makeText(getApplicationContext(), "Ten kontakt nie zawiera numeru", Toast.LENGTH_LONG).show();
                }
                c.close();
              }
              break;
